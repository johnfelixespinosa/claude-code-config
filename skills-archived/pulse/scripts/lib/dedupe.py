"""Near-duplicate detection for pulse skill."""
...
def get_item_text(item: Union[schema.RedditItem, schema.XItem, schema.HackerNewsItem, schema.GitHubItem]) -> str:
    ...
    if isinstance(item, schema.RedditItem):
        ...
        return f"{item.full_name} {item.description}"
    else:
        return ""


def find_duplicates(
    items: List[Union[schema.RedditItem, schema.XItem, schema.HackerNewsItem, schema.GitHubItem]],
    threshold: float = 0.7,
) -> List[Tuple[int, int]]:
...
def dedupe_items(
    items: List[Union[schema.RedditItem, schema.XItem, schema.HackerNewsItem, schema.GitHubItem]],
    threshold: float = 0.7,
) -> List[Union[schema.RedditItem, schema.XItem, schema.HackerNewsItem, schema.GitHubItem]]:
