"""Popularity-aware scoring for pulse skill."""
...
def compute_github_engagement_raw(engagement: Optional[schema.Engagement]) -> Optional[float]:
    ...
    Formula: 0.70*log1p(stars) + 0.30*log1p(forks)
    """
    if engagement is None:
        return None

    if engagement.stars is None and engagement.forks is None:
        return None

    stars = log1p_safe(engagement.stars)
    forks = log1p_safe(engagement.forks)

    return 0.70 * stars + 0.30 * forks
...
def score_hn_items(items: List[schema.HackerNewsItem]) -> List[schema.HackerNewsItem]:
    ...
    Uses same 45/25/30 weights as Reddit/X but with -5pt source penalty.

    Args:
        items: List of HackerNews items
    ...
    for i, item in enumerate(items):
        ...
        # Apply HN source penalty
        overall -= HN_SOURCE_PENALTY
...
def score_github_items(items: List[schema.GitHubItem]) -> List[schema.GitHubItem]:
    ...
    Uses same 45/25/30 weights as Reddit/X but with -10pt source penalty.
    ...
    for i, item in enumerate(items):
        ...
        # Apply GitHub source penalty
        overall -= GITHUB_SOURCE_PENALTY
...
def sort_items(items: List[Union[schema.RedditItem, schema.XItem, schema.HackerNewsItem, schema.GitHubItem, schema.WebSearchItem]]) -> List:
    ...
    def sort_key(item):
        ...
        # Tertiary: source priority (Reddit > X > HN > GitHub > WebSearch)
        ...
        text = getattr(item, "title", "") or getattr(item, "text", "") or getattr(item, "full_name", "")
