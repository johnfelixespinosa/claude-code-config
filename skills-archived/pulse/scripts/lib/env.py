...
def get_config() -> Dict[str, Any]:
    """Load configuration from ~/.config/pulse/.env and environment."""
...
def validate_sources(requested: str, available: str, include_web: bool = False) -> tuple[str, Optional[str]]:
    ...
    if available == 'web':
        if requested == 'auto':
            ...
            return 'web', f"No API keys configured. Using WebSearch fallback. Add keys to ~/.config/pulse/.env for Reddit/X."
