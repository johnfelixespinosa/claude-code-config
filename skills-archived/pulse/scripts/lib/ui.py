...
BANNER = f"""{Colors.PURPLE}{Colors.BOLD}
  ...
  ██║     ╚██████╔╝███████╗███████║███████╗
  ╚═╝      ╚═════╝ ╚══════╝╚══════╝╚══════╝
{Colors.RESET}{Colors.DIM}  Reddit + X + HackerNews + GitHub + Web{Colors.RESET}
"""
...
HN_MESSAGES = [
    "Browsing HackerNews frontpage...",
    "Reading the orange site...",
    "Finding Show HN and Ask HN posts...",
    "Scanning HN stories...",
    "Checking what hackers are discussing...",
    "Exploring the tech community...",
]

GITHUB_MESSAGES = [
    "Searching GitHub repositories...",
    "Finding starred repos...",
    "Browsing open source projects...",
    "Scanning for trending repos...",
...
{Colors.YELLOW}UNLOCK THE FULL POWER OF /pulse{Colors.RESET}

{Colors.DIM}HackerNews + GitHub always run (free). Add API keys for more:{Colors.RESET}
...
{Colors.DIM}Setup:{Colors.RESET} Edit {Colors.BOLD}~/.config/pulse/.env{Colors.RESET}
{Colors.YELLOW}{Colors.BOLD}--------------------------------------------------------------------{Colors.RESET}
"""

PROMO_MESSAGE_PLAIN = """
--------------------------------------------------------------------
UNLOCK THE FULL POWER OF /pulse

HackerNews + GitHub always run (free). Add API keys for more:
...
PROMO_SINGLE_KEY = {
    "reddit": f"""
{Colors.DIM}Tip: Add {Colors.YELLOW}OPENAI_API_KEY{Colors.RESET}{Colors.DIM} to ~/.config/pulse/.env for Reddit data with real engagement metrics!{Colors.RESET}
""",
    "x": f"""
{Colors.DIM}Tip: Add {Colors.CYAN}XAI_API_KEY{Colors.RESET}{Colors.DIM} to ~/.config/pulse/.env for X/Twitter data with real likes & reposts!{Colors.RESET}
""",
}

PROMO_SINGLE_KEY_PLAIN = {
    "reddit": "\nTip: Add OPENAI_API_KEY to ~/.config/pulse/.env for Reddit data with real engagement metrics!\n",
    "x": "\nTip: Add XAI_API_KEY to ~/.config/pulse/.env for X/Twitter data with real likes & reposts!\n",
}
...
class ProgressDisplay:
    ...
    def start_hn(self):
        ...
        self.spinner = Spinner(f"{Colors.GREEN}HackerNews{Colors.RESET} {msg}", Colors.GREEN)
        self.spinner.start()

    def end_hn(self, count: int):
        if self.spinner:
            self.spinner.stop(f"{Colors.GREEN}HackerNews{Colors.RESET} Found {count} stories")

    def start_github(self):
        msg = random.choice(GITHUB_MESSAGES)
        self.spinner = Spinner(f"{Colors.BLUE}GitHub{Colors.RESET} {msg}", Colors.BLUE)
        self.spinner.start()

    def end_github(self, count: int):
        if self.spinner:
            self.spinner.stop(f"{Colors.BLUE}GitHub{Colors.RESET} Found {count} repos")
    ...
    def show_complete(self, reddit_count: int, x_count: int, hn_count: int = 0, github_count: int = 0):
        ...
        if IS_TTY:
            ...
            parts = []
            if reddit_count > 0:
                parts.append(f"{Colors.YELLOW}Reddit:{Colors.RESET} {reddit_count} threads")
            if x_count > 0:
                parts.append(f"{Colors.CYAN}X:{Colors.RESET} {x_count} posts")
            if hn_count > 0:
                parts.append(f"{Colors.GREEN}HN:{Colors.RESET} {hn_count} stories")
            if github_count > 0:
                parts.append(f"{Colors.BLUE}GitHub:{Colors.RESET} {github_count} repos")
            if parts:
                sys.stderr.write("  " + "  ".join(parts) + "\n\n")
        else:
            parts = []
            if reddit_count > 0:
                parts.append(f"Reddit: {reddit_count} threads")
            if x_count > 0:
                parts.append(f"X: {x_count} posts")
            if hn_count > 0:
                parts.append(f"HN: {hn_count} stories")
            if github_count > 0:
                parts.append(f"GitHub: {github_count} repos")
    ...
    def show_lite_complete(self, hn_count: int, github_count: int):
        """Show completion for lite mode (HN + GitHub only)."""
        elapsed = time.time() - self.start_time
        if IS_TTY:
            sys.stderr.write(f"\n{Colors.GREEN}{Colors.BOLD}Lite research complete{Colors.RESET} ")
            sys.stderr.write(f"{Colors.DIM}({elapsed:.1f}s){Colors.RESET}\n")
            sys.stderr.write(f"  {Colors.GREEN}HN:{Colors.RESET} {hn_count} stories  ")
            sys.stderr.write(f"{Colors.BLUE}GitHub:{Colors.RESET} {github_count} repos\n\n")
        else:
            sys.stderr.write(f"Lite research complete ({elapsed:.1f}s) - HN: {hn_count} stories, GitHub: {github_count} repos\n")
        sys.stderr.flush()
