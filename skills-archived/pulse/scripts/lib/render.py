"""Output rendering for pulse skill."""
...
OUTPUT_DIR = Path.home() / ".local" / "share" / "pulse" / "out"
...
def _assess_data_freshness(report: schema.Report) -> dict:
    ...
    total_recent = reddit_recent + x_recent + hn_recent + gh_recent + web_recent
    total_items = len(report.reddit) + len(report.x) + len(report.hackernews) + len(report.github) + len(report.web)
...
def render_compact(report: schema.Report, limit: int = 15, missing_keys: str = "none") -> str:
    ...
    # Web-only mode banner (when no API keys AND no HN/GitHub)
    if report.mode == "web-only":
        ...
        lines.append("- Edit `~/.config/pulse/.env` to add keys")
        lines.append("---")
        lines.append("")

    # Lite mode banner
    if report.mode == "lite":
        lines.append("**LITE MODE** - HackerNews + GitHub + Web (no API keys needed)")
        lines.append("")
        lines.append("---")
        lines.append("**Want Reddit & X data?** Add API keys to `~/.config/pulse/.env`:")
        lines.append("- `OPENAI_API_KEY` -> Reddit threads with real upvotes & comments")
        lines.append("- `XAI_API_KEY` -> X posts with real likes & reposts")
        lines.append("---")
        lines.append("")
    ...
    if report.reddit_error:
        ...
        lines.append("")
    elif report.mode in ("both", "reddit-only", "all", "reddit-web") and not report.reddit:
        lines.append("### Reddit Threads")
        lines.append("")
    ...
    # HackerNews items
    if report.hn_error:
        lines.append("### HackerNews Stories")
        lines.append("")
        ...
        for item in report.github[:limit]:
            ...
            if item.engagement:
                ...
                if eng.stars is not None:
                    parts.append(f"{eng.stars}stars")
            ...
            lines.append(f"**{item.id}** (score:{item.score}) {item.full_name}{lang_str}{date_str}{conf_str}{eng_str}")
            if item.description:
...
def render_context_snippet(report: schema.Report) -> str:
    ...
    for item in report.github[:5]:
        all_items.append((item.score, "GitHub", item.full_name, item.url))
...
def render_full_report(report: schema.Report) -> str:
    ...
    # Title
    lines.append(f"# {report.topic} - Pulse Research Report")
    lines.append("")
    ...
    if report.hackernews:
        ...
        for item in report.hackernews:
            ...
            lines.append(f"- **HN URL:** {item.url}")
            if item.story_url:
                lines.append(f"- **Story URL:** {item.story_url}")
            lines.append(f"- **Date:** {item.date or 'Unknown'} (confidence: {item.date_confidence})")
            lines.append(f"- **Score:** {item.score}/100")
    ...
    if report.github:
        ...
        for item in report.github:
            ...
            lines.append(f"- **Language:** {item.language or 'N/A'}")
            lines.append(f"- **URL:** {item.url}")
            lines.append(f"- **Date:** {item.date or 'Unknown'} (confidence: {item.date_confidence})")
            lines.append(f"- **Score:** {item.score}/100")
            ...
            if item.engagement:
                eng = item.engagement
                lines.append(f"- **Engagement:** {eng.stars or '?'} stars, {eng.forks or '?'} forks")
...
def write_outputs(
    ...
    Args:
        ...
        raw_hn: Raw HN API response
        raw_github: Raw GitHub API response
    """
    ensure_output_dir()
    ...
    # pulse.context.md
    with open(OUTPUT_DIR / "pulse.context.md", 'w') as f:
    ...
    if raw_github:
        with open(OUTPUT_DIR / "raw_github.json", 'w') as f:
            json.dump(raw_github, f, indent=2)


def get_context_path() -> str:
    """Get path to context file."""
    return str(OUTPUT_DIR / "pulse.context.md")
