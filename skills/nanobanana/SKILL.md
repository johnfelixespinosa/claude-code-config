...
Use Gemini CLI plus the Nano Banana extension to generate and edit images via slash commands. Outputs are saved to `~/Desktop/Projects/NanaImages/` for review or delivery.
...
3. Create the output directory if it doesn't exist: `mkdir -p ~/Desktop/Projects/NanaImages`
4. Start Gemini CLI in the output directory: `cd ~/Desktop/Projects/NanaImages && gemini --approval-mode yolo -p "..."` for one-shot runs
...
6. Find the generated image(s) in `~/Desktop/Projects/NanaImages/nanobanana-output/`
7. Open the image in Preview immediately: `open <image_path>`
